<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden rounded-lg grid grid-cols-3 gap-4 mt-4 shadow-sm">
                <div class="flex justify-center">
                    <p class="px-6 py-4 text-sm md:text-lg">Cliente</p>
                </div>
                <div class="flex justify-center">
                    <p class="px-6 py-4 text-sm md:text-lg">Valor</p>
                </div>
                <div class="flex justify-center">
                    <p class="px-6 py-4 text-sm md:text-lg">Ações</p>
                </div>
            </div>
            <div>
                @if ($loanlist->count() == 0)
                ...
                @foreach ($loanlist as $loan)
                    @php
                        ...
                        $user = \App\Models\Manager::where('account', $recipientAccount->id)->first();
                        $address = $user->address()->first();
                        
                    @endphp
                    <div class="bg-white overflow-hidden rounded-lg grid grid-cols-3 gap-4 mt-4 shadow-sm p-4 mb-2">
                        <div class="flex justify-center items-center">
                            <button onclick="openUserModal('userModal-{{$loan->id}}')">
                                <p class="text-[#0571d3] hover:text-orange-400 transition-colors text-sm md:text-base">
                                    {{ $user->name }}
                                </p>
                            </button>
                        </div>
                        <div class="flex justify-center items-center">
                            <p class="text-sm md:text-base">{{ "R$ " . number_format($loan->value, 2, ',', '.') }}</p>
                        </div>
                        <div class="flex gap-2 ml-4 justify-center items-center">
                            <form action="{{route('admin-accept-loan', $loan->id)}}" method="POST">
                                @csrf
                                @method('POST')
                                <button type="submit"
                            ...
                            <form action="{{route('admin-deny-loan', $loan->id)}}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                    ...
                    <div id="userModal-{{$loan->id}}" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center">
                            ...
                            <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-2 w-full">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <input type="text" id="transferLimit" readonly
                                        value="{{ 'R$ ' . number_format($recipientAccount->transferLimit, 2, ',', '.') }}"
                                        class="rounded-md">
                                </div>
                                <div class="flex flex-col">
                                    <label for="admin">Administrador</label>
                                    <input type="text" id="admin" readonly value="{{ $admin->name }}"
                                        class="rounded-md">
                                </div>
                            </div>
                            <button onclick="closeUserModal('userModal-{{$loan->id}}')" class="bg-red-500 hover:bg-orange-400 transition-colors text-white text-[17px] overflow-hidden shadow-sm rounded-lg flex justify-center items-center p-5 w-fit h-fit mt-4">
                                Fechar
                            </button>
                        </div>
                    </div>
